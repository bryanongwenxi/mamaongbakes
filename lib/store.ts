...
async function init(){ if(!sql)return; if(!initialization)initialization=(async()=>{
 ...
 await sql`CREATE TABLE IF NOT EXISTS mob_login_limits (key text PRIMARY KEY, attempts integer NOT NULL, expires_at timestamptz NOT NULL)`;
...
const localLimits=new Map<string,{attempts:number;expires:number}>();
export async function allowLogin(key:string){
 if(sql){await init();await sql`DELETE FROM mob_login_limits WHERE expires_at < now()`;const [r]=await sql`INSERT INTO mob_login_limits (key,attempts,expires_at) VALUES (${key},1,now()+interval '15 minutes') ON CONFLICT (key) DO UPDATE SET attempts=mob_login_limits.attempts+1 RETURNING attempts`;return r.attempts<=8;}
 if(process.env.NODE_ENV==='production')return false;
 const now=Date.now();for(const [k,v]of localLimits)if(v.expires<now)localLimits.delete(k);const v=localLimits.get(key)??{attempts:0,expires:now+900000};v.attempts++;localLimits.set(key,v);return v.attempts<=8;
}
